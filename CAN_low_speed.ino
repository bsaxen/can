...
void loop() {
  ...
  if (now - t10 >= 100) {
      ...
      unsigned char temp_status[8] = {0x82, 0x00, 0xFF, 0x00, 0x00, 0x00, 0x00, 0x00};
      CAN.sendMsgBuf(0x420, 0, 8, temp_status);
  
      unsigned char clock[8] = {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00};
      CAN.sendMsgBuf(0x3B3, 0, 8, clock);

      unsigned char radiotext[8] = {0x00, 0x11, 0x66, 0x00, 0x00, 0x00, 0x00, 0x00};
      CAN.sendMsgBuf(0x3E0, 0, 8, radiotext);

      unsigned char outtemp[8] = {0x03, 0x04, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00};
      CAN.sendMsgBuf(0x275, 0, 8, outtemp);
